...
export function CodeBlockCommand({
  ...
  return (
    <div className="overflow-x-auto relative px-2 py-1 rounded-md bg-gray-50 mt-4">
      <Tabs
        className="gap-0"
        defaultValue="pnpm"
        value={packageManager}
        onValueChange={(v) => setPackageManager(v as typeof packageManager)}
      >
          ...
          {Object.entries(tabs).map(([key, value]) => {
            return (
              <TabsContent key={key} value={key} className="mt-0 px-4 py-3.5">
                <pre>
                  <code
                    className="relative font-mono text-sm leading-none not-prose"
                    data-language="bash"
                  >
                    {value}
                  </code>
